...
pub fn get_with_size(name: String, size: Int) -> element.Element(a) {
  create_icon(name, option.Some(size))
}

pub fn get(name: String) -> element.Element(a) {
  create_icon(name, option.None)
}

fn create_icon(name: String, size: option.Option(Int)) -> element.Element(a) {
  let icon_size = option.unwrap(size, 16)
  let assert Ok(icon) = list.key_find(icons_list(), name)

  svg.svg(
    [
      attr.width(icon_size),
      attr.height(icon_size),
      attr.attribute("viewBox", icon.viewbox),
      attr.attribute("fill", "currentColor"),
      attr.class("flex-shrink-0"),
    ],
    [
      svg.path([
        attr.attribute("d", icon.path),
      ]),
    ],
  )
}

pub fn icons_list() -> List(#(String, Icon)) {
  [
    #(
      "calendar",
      Icon(
